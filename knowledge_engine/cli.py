...
def cmd_ingest_image(args, cfg):
    from knowledge_engine.ingest.pipeline import ingest_image
    con = _con(cfg)
    r = ingest_image(con, args.path, cfg=cfg)
    _print_ingest(r)
    con.close()
...
def cmd_timeline(args, cfg):
    ...
    try:
        if args.date:
            rows = con.execute(
                "SELECT id, kind, title, raw_path, captured_at FROM sources "
                "WHERE date(captured_at) = date(?) ORDER BY captured_at DESC",
                (args.date,),
            ).fetchall()
        else:
            rows = con.execute(
        ...
        for r in rows:
            ...
            if day != cur_day:
                print(f"\n== {day} ==")
                cur_day = day
            print(f"  #{r['id']} [{r['kind']}] {(r['title'] or '')[:60]}")
    finally:
        con.close()
...
def main(argv=None):
    ...
    sp = sub.add_parser("ingest-image", help="从图片 OCR 采集")
    sp.add_argument("path")
    sp.set_defaults(fn=cmd_ingest_image)
    ...
    sp = sub.add_parser("timeline", help="采集时间线（按日分组）")
    sp.add_argument("--date", default=None, help="YYYY-MM-DD 过滤当天；不传返回最近记录")
